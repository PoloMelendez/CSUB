...
int main(int argc, char *argv[], char *envp[])
{
  ...
  fprintf(stdout, "\nMy PID is: %d\n", my_pid);
  fprintf(stdout, "Send SIGUSR1 to pause, SIGUSR2 to cont or Ctrl-C to stop\n");
  while (1) {
     if (DOTS) {
       usleep(1200000);  /* usleep is in microseconds - this is 1.2 seconds ;
...
void usr2_handler(int signo) 
{
  if (paused == 0) 
  {
     ...
     sigprocmask(SIG_BLOCK, &handlermask, &savedmask); /* block usr1,usr2, int
                                                        signals */
